...
import { CartReference, CustomerReference, OrderReference } from '@commercetools/platform-sdk'
import { ctLogin } from "services/Commercetools"
  ...
  async #config() {
    ...
    this.#app.use(
      (
        ...
        const action = req.body.resource as CartReference | OrderReference | CustomerReference
        ...
        if (!talonOneUtils) {
          logger.warn(`There is no TalonOne API_KEY or API_URL for the [${currencyCode}] currency code in the environment variables.`)
